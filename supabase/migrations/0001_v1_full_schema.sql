...
-- Structure: all tables first, then all RLS policies, then functions/triggers.
-- =====================================================================
...
-- (All tables exist now — cross-table references are safe)
-- =====================================================================
...
-- No update/delete policies on books — append-only cache.
...
create policy "pool_books_select_member"
  on public.pool_books for select to authenticated
  using (
    exists (
      select 1 from public.club_members cm
...
create policy "pool_books_insert_member"
  on public.pool_books for insert to authenticated
  with check (
...
-- vote_count updated ONLY by increment_book_vote RPC (SECURITY DEFINER).
-- No update policy means clients cannot UPDATE vote_count directly.

create policy "pool_books_delete_admin"
  on public.pool_books for delete to authenticated
  using (
    exists (
      select 1 from public.club_members cm
...
-- NO INSERT/UPDATE/DELETE policies on votes — clients cannot write directly.
-- increment_book_vote RPC uses SECURITY DEFINER to bypass RLS for inserts.
...
-- increment_book_vote RPC  (VOTE-02, VOTE-04 — Phase 4)
...
-- =====================================================================
-- SECTION 5: REALTIME PUBLICATION  (VOTE-03, MEET-02 — Phase 4/5)
-- =====================================================================
